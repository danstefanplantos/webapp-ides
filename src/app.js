...
searchInput.addEventListener("input", () => {
  ...
  const foundScenario = scenarios.find(s =>
    s.title.toLowerCase().includes(query) ||
    s.description.toLowerCase().includes(query) ||
    s.crime.toLowerCase().includes(query)
  );

  if (foundScenario && query.length > 2) {
    showScenario(foundScenario);
  }
});

if (!foundScenario && query.length > 2) {
  resultBox.style.display = "block";
  resTitle.textContent = "Nessun risultato";
  resDescription.textContent = "Non è stato trovato alcun caso corrispondente.";
}